...
class AgentState(TypedDict):
    ...
    result_df_json: str         # JSON of processed/filtered result DataFrame for download
...
def analyze_node(state: AgentState) -> AgentState:
    ...
    # Build a column-prefix map so the LLM knows what prefix to apply per dataset.
    # e.g. df_sales_2024 → "sales_2024", df → "data"
    prefix_map = {
        vn: (vn[3:] if vn.startswith("df_") else ("data" if vn == "df" else vn))
        for vn in dfs.keys()
    }
    prefix_lines = "\n".join(
        f"    {vn!r} → column prefix \"{pfx}_\""
        for vn, pfx in prefix_map.items()
    )
    next_rule = 9 if multi else 8
...
{next_rule}. Also produce a variable called `result_df` — a pandas DataFrame of the actual \
processed/filtered/joined records that answer the query:
   - Use these column prefixes so every column is traceable to its source dataset:
{prefix_lines}
   - Rename ALL columns of result_df with the appropriate prefix before assigning.
     Example (single dataset df_sales_2024):
       result_df = top3.rename(columns={{c: f"sales_2024_{{c}}" for c in top3.columns}})
     Example (join of df_sales and df_customers):
       s = df_sales.rename(columns={{c: f"sales_{{c}}" for c in df_sales.columns}})
       c = df_customers.rename(columns={{c: f"customers_{{c}}" for c in df_customers.columns}})
       result_df = s.merge(c, left_on="sales_Customer_ID", right_on="customers_Customer_ID")
   - If the answer is a pure scalar (e.g. a single count), set:
       result_df = pd.DataFrame([{{"result": <value>}}])
   - Keep result_df as a proper DataFrame (do not convert to string)
...
Return ONLY the Python code, no markdown fences or explanations."""
    ...
    try:
        ...
        # Capture and serialise the processed DataFrame if provided
        result_df = local_vars.get("result_df")
        if isinstance(result_df, pd.DataFrame) and not result_df.empty:
            # Coerce all column names to strings (safety for mixed-type columns)
            result_df.columns = [str(c) for c in result_df.columns]
            result_df_json = result_df.to_json(orient="records", date_format="iso", default_handler=str)
    except Exception as e:
